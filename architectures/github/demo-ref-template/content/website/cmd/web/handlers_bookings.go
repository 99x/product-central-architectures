...
import (
	...
	modelsShowTime "github.com/99x-product-central-dev/microservices-docker-go-mongodb/showtimes/pkg/models"
	modelsUser "github.com/99x-product-central-dev/microservices-docker-go-mongodb/users/pkg/models"
	"github.com/gorilla/mux"
)

type bookingTemplateData struct {
	${{values.firstAppName}}      models.${{values.firstAppName}}
	Bookings     []models.${{values.firstAppName}}
...
func (app *application) loadBookingData(btd *bookingTemplateData, isList bool) {
	// Clean ${{values.firstAppName}} data
	btd.BookingsData = []bookingData{}
	btd.BookingData = bookingData{}

	// Load ${{values.firstAppName}} data
	if isList {
...
func (app *application) bookingsView(w http.ResponseWriter, r *http.Request) {
	...
	err := app.getAPIContent(url, &td.${{values.firstAppName}})
	if err != nil {
		app.errorLog.Println(err.Error())
	}
	app.infoLog.Println(td.${{values.firstAppName}})
